...
int sdhc_spi_send_cmd(struct sdhc_spi_device *dev, struct sdhc_command *cmd, int data_present)
{
	...
	LOG_DBG("Send Command 0x%x", scratch);
...
int sdhc_spi_write_data(const struct sdhc_spi_device *dev, struct sdhc_data *data)
{
	...
	if (data->blocks > 1)
	{
		...
		// Send an additional stuff byte after the token to ensure
		// MCU correctly waits for SD to become unbusy
		uint8_t buf[2] = { token, 0xFF };
		ret = sdhc_spi_tx(hspi, buf, 2);
		if (ret != HAL_OK)
		{
			return -EIO;
		}
